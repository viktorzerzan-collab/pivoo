...
try {
    ...
    $stmt = $api->db->prepare($beers_query);
    $stmt->execute($params);
...
try {
    ...
    $stmt = $api->db->prepare($brew_query);
    $stmt->execute($params);
...
try {
    ...
    $stmt = $api->db->prepare($days_query);
    $stmt->execute($params);
...
try {
    ...
    $stmt = $api->db->prepare($months_query);
    $stmt->execute($params);
...
try {
    $month_days_query = "SELECT DAY(c.consumed_at) as day_index, SUM(c.quantity) as count
                         ...
                         WHERE $userCondition $dateCondition
                         GROUP BY day_index ORDER BY day_index ASC";
    $stmt = $api->db->prepare($month_days_query);
    $stmt->execute($params);
...
try {
    $overview_query = "SELECT 
                       ...
                       FROM consumptions c
                       WHERE $userCondition $dateCondition";
    $stmt = $api->db->prepare($overview_query);
    $stmt->execute($params);
...
try {
    ...
    $stmt = $api->db->prepare($styles_query);
    $stmt->execute($params);
...
try {
    ...
    $stmt = $api->db->prepare($price_query);
    $stmt->execute($params);
    $res = $stmt->fetch(PDO::FETCH_ASSOC);
...
$api->response->sendSuccess("", ["data" => $response]);
?>