...
// Importujeme skenovací knihovnu
...
const startBarcodeScanner = () => {
  ...
  // Timeout zajistí, že DOM (div #barcode-reader) bude plně zobrazen
  setTimeout(() => {
    html5QrCode = new Html5Qrcode("barcode-reader")
    html5QrCode.start(
      { facingMode: "environment" }, // Priorita na zadní kameru
      { 
        fps: 10, 
      ...
      (errorMessage) => {
        // Zde ignorujeme běžné chyby čtení na pozadí, dějí se neustále dokud kód není perfektně zachycen
      }
    ).catch((err) => {
...
const handleBarcodeScanned = async (ean) => {
  ...
  try {
    ...
    if (res.status === 'success') {
      emit('result', res) // Předáme data hlavnímu modálu, který je propíše!
      magicMessage.value = ''
